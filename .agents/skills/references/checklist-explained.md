...
## 4. Zod .strict() on every route body
...
// ✅ Fastify schema with additionalProperties
fastify.post('/', {
  schema: {
    body: {
      type: 'object',
...
{
  "scripts": {
    "check:security": "npm run check:console && npm run check:types && npm run check:storeid",
