...
export const authService = {
  ...
  async generateMfaCode(
    ...
    // randomInt(100000, 1000000) is cryptographically secure (uses node:crypto
    // under the hood) and uniformly distributed across the 6-digit code space.
  ...
  async verifyMfaCode(
    ...
    // timingSafeEqual requires equal-length Buffers. The stored code is always
    // 6 digits (generated by randomInt above), and we pad the input to the
    // same length to avoid leaking length-based information. If the input
    // is not 6 digits the buffers will differ in length and we return false.
    const storedBuf = Buffer.from(stored, 'utf8');
    const codeBuf = Buffer.from(code.padEnd(6, '\0').slice(0, 6), 'utf8');
    if (storedBuf.length !== codeBuf.length) {
      return false;
    }
    const match = timingSafeEqual(storedBuf, codeBuf);
    if (!match) {
      return false;
    }
