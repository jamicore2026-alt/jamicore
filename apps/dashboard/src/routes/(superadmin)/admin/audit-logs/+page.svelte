<script lang="ts">
	...
	const actionColors: Record<string, string> = {
		...
		logout: 'bg-gray-500/15 text-gray-500 border-gray-500/30',
		view: 'bg-amber-500/15 text-amber-500 border-amber-500/30',
	};
...
<div class="space-y-8 stagger-children">
	...
	<div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
		<div>
			<div class="flex items-center gap-2 mb-2">
				...
				<span class="text-[11px] font-mono uppercase tracking-widest text-primary">Observability</span>
			</div>
			<h1 class="text-3xl font-bold tracking-tight font-heading">Audit Logs</h1>
			<p class="text-muted-foreground mt-1 text-sm">Platform activity and audit trail.</p>
		</div>
	</div>
	...
	<Card class="glass-card">
		<CardHeader class="pb-3">
			<CardTitle class="text-base font-semibold font-heading">{total} log{total !== 1 ? 's' : ''}</CardTitle>
		</CardHeader>
			...
			{#if logs.length === 0}
				<div class="flex flex-col items-center justify-center py-16 text-center">
					<div class="h-16 w-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center mb-4" style="box-shadow: 0 0 24px rgba(6,182,212,0.05);">
						<ScrollText class="h-7 w-7 text-muted-foreground/40" />
					</div>
					<p class="text-muted-foreground font-medium font-heading">No logs yet</p>
						...
						<Table.Body>
							{#each logs as log (log.id)}
								<Table.Row>
									...
									<Table.Cell>
										<Badge class="{actionColors[log.action] || 'bg-gray-500/15 text-gray-500 border-gray-500/30'} capitalize whitespace-nowrap text-xs">{log.action}</Badge>
									</Table.Cell>
