...
- Name: admin\store-approval.spec.ts >> Admin Store Approval >> admin can view pending stores
- Location: e2e\specs\admin\store-approval.spec.ts:4:3
...
```ts
  ...
  2  | 
  3  | test.describe('Admin Store Approval', () => {
  4  |   test('admin can view pending stores', async ({ adminPage }) => {
  5  |     await adminPage.goto('/admin/stores');
> 6  |     await expect(adminPage).toHaveTitle(/Stores/);
     |                             ^ Error: expect(page).toHaveTitle(expected) failed
  7  |     await expect(adminPage.locator('text=Organic Market')).toBeVisible();
  8  |   });
  9  | });
  10 | 
```