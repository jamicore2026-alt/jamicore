...
function _page$1($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		derived(() => storeSlug() ? `/store/${storeSlug()}/brio/cart` : "/cart");
		let qty = 1;
		...
		if (!product()) {
			...
			for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
				...
				$$renderer.push(`<button class="flex-1 py-2 px-3 border text-sm font-medium transition-colors"${attr_style(`border-color: ${stringify(selectedSpice === spice.id ? "#1a4d2e" : "#e5e5e5")}; background-color: ${stringify(selectedSpice === spice.id ? "#e8f5e9" : "#ffffff")}; color: ${stringify(selectedSpice === spice.id ? "#1a4d2e" : "#1a1a1a")}; border-radius: 4px;`)}>${escape_html(spice.label)}</button>`);
			}
