...
- **Fix:** All third-party actions in `ci.yml`, `e2e.yml`, and `build-images.yml` are now pinned to specific commit SHAs with version comments (e.g., `actions/checkout@11bd71901bbe5b1630ceea73d27597364c9af683 # v4.2.2`).
...
- **Fix:** The app now uses `fastify.decorateRequest('requestId', '')` for type-safe decoration. The property is declared in the Fastify type declaration file so all consumers get compile-time type checking.
...
- **Fix:** Both `apps/storefront/src/lib/config/env.ts` and `apps/dashboard/src/lib/config/env.ts` now validate required env vars with Zod at module load time and throw on invalid configuration.
...
- **Fix:** The Pino logger now includes `redact` for `req.headers.authorization`, `req.body.password`, `req.body.token`, `req.body.cardNumber`, `req.body.cvv`, `req.body.secret`, `req.body.apiKey`, `req.body.webhook_secret`, and `req.body.secret_key`.
...
- **Fix:** `/health/ready` now checks `getPoolMetrics()`. If `poolMetrics.waiting > 5` or `poolMetrics.active / poolMax > 0.9`, it returns HTTP 503 with `{ status: 'not ready', error: 'DB pool saturated' }`.
...
- **Fix:** `docker-compose.yml` now includes a `backend` service using `node:22-alpine` with bind-mounted volumes for live reload, `.env` file loading, health checks matching production, and `depends_on` with `condition: service_healthy` for both Postgres and Redis.
...
- **Fix:** `packages/shared-types/package.json` now defines subpath exports for `.`, `./schemas`, and `./types`, enabling tree-shaking and targeted imports.
...
- **Fix:** All routes are already scoped under `/api/v1/...` prefixes (public, merchant, customer, admin). The swagger spec version `1.0.0` aligns with the v1 URL namespace.
...
- **Fix:** `@axe-core/playwright` is integrated via the `@repo/e2e-utils` package. E2E tests for homepage, product detail, and checkout flow all run `expectNoAccessibilityViolations(page)` using AxeBuilder.
...
- **Fix:** The `servers` array is now dynamic: it always includes the local dev URL, conditionally adds `API_BASE_URL` (labeled as Production or Staging), and conditionally adds the Storefront BFF proxy path when `STOREFRONT_URL` is configured.
